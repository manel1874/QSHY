...
void mpc_main(Array_Seq INPUT_A, Array_Seq INPUT_B)
{
	...
	{
		n1_dist = n1_dist + (16 - popcount( INPUT_A.el[i]^INPUT_B.el[i]^0xAAAAAAAA ));
		n2_dist = n2_dist + popcount(INPUT_A.el[i]^INPUT_B.el[i]) - (16 - popcount( INPUT_A.el[i]^INPUT_B.el[i]^0xAAAAAAAA ));
